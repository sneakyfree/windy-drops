...
The TypeScript sibling on npm (`npm install -g @windy/drops-sdk`) has the same CLI surface. A drop scaffolded by one is byte-identical to one scaffolded by the other; a drop published by one is indistinguishable to the registry. Cross-SDK byte-identity is enforced by the `WD-11` conformance harness.
